...
(defmacro dispatch-mime-type** (implementation header &body body)
  (let* (on-unhandled
        ...
        `(let ((,type-handlers ',(mapcar #'(lambda (item)
           ...
           (let ((,handler (reduce (named-lambda find-matched-type! (result item)
             ...
             (case (cdr ,handler)
               ,@(mapcar #'(lambda (item)
                             `(,(second item)
                                (setf (http-header :CONTENT-TYPE ,impl) ,(format-mime-type (first item)))
                                ,@(third item)))
                         handlers)
               ,@(when on-unhandled
                       `((t ,@on-unhandled)))
...
(defmacro dispatch-mime-type* (implementation &body body)
  (once-only ((impl implementation))
    `(progn
       ...
       (dispatch-mime-type** ,impl (http-header :ACCEPT ,impl)
         ,@body))))
